...
def main():
    ...
    id_mask   = readid('Mask_ID.FC')
    id_remove = readtable('Remove_ID.FC')
    agp = 'Fix_region.NewRef.fasta.gap/PGA.agp'
    contig = fasta_seq('Fix_region.NewRef.fasta.gap/PGA.contigs.fa')
    prefix = 'Fix_region.NewRef.fasta.gap/PGA'
    maskregion(prefix, id_mask, agp, contig, id_remove)
    agp_temp = '%s.temp.agp' %(prefix)
    update_agp(prefix, agp_temp, id_remove)
